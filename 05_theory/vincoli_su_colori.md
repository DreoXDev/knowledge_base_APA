---
type: theory
topic: vincoli-su-colori
status: scaffold
tags:
  - apa
  - teoria
  - topic/colori
  - topic/stato-esteso
---

# Teoria - Vincoli su colori

## Definizione minima

Vincoli che dipendono dal colore di simboli o archi e richiedono di aggiungere contatori, budget o informazioni locali allo stato del sottoproblema.

## Quando compare negli appelli

- LCS con numero massimo di simboli rossi e blu.
- Cammini in grafi colorati con conteggi esatti, vincoli di precedenza, somme aggregate o divieti consecutivi.
- LCS con presenza obbligatoria di almeno un simbolo rosso.
- LCS a tre sequenze con budget massimo di simboli rossi.

## Collegamenti agli esercizi

- [[exam_2025_06_09_p1_e01]]
- [[exam_2025_06_09_p1_e02]]
- [[exam_2025_07_03_p1_e02]]
- [[exam_2025_11_10_p1_tema_a_e01]]
- [[exam_2025_11_10_p1_tema_a_e02]]
- [[exam_2025_02_11_p1_completo_e01]]
- [[exam_2025_02_11_p1_completo_e02]]
- [[exam_2025_02_11_p1_recupero_e01]]
- [[exam_2025_02_11_p1_recupero_e02]]

## Collegamenti ai metodi

- [[metodo_programmazione_dinamica_lcs_vincoli_colori]]
- [[metodo_lcs_tre_sequenze_vincolo_colori]]
- [[metodo_dp_cammini_colori_conteggi]]
- [[metodo_dp_cammini_colori_precedenze]]
- [[metodo_cammini_minimi_vincoli_colori_parita]]

> [!Warning]
> Nota scaffold: completare solo dopo aver risolto almeno un esercizio per variante.

